...
static const rr::Vec3 cubeVerts[] = {
    {{-1.0f, 1.0f, 1.0f}},
    {{-1.0f,-1.0f, 1.0f}},
    {{ 1.0f,-1.0f, 1.0f}},
    {{ 1.0f, 1.0f, 1.0f}},
...
static rr::Vec3 cubeAdjacencies[6 * 12];
...
public:
    ...
    void init(const uint32_t resolutionW, const uint32_t resolutionH)
    {
        ...
        createAdjacentList(cubeAdjacencies, cubeVerts, cubeIndex, 12);
        // for (uint32_t i = 0; i < 12; i++)
        // {
        //     qDebug() << "Face " << i;
        //     qDebug() 
        //         << "{"
        //         << cubeAdjacencies[(i * 6) + 0][0] << ", "
        //         << cubeAdjacencies[(i * 6) + 0][1] << ", "
        //         << cubeAdjacencies[(i * 6) + 0][2] << "}, {"
        //         << cubeAdjacencies[(i * 6) + 1][0] << ", "
        //         << cubeAdjacencies[(i * 6) + 1][1] << ", "
        //         << cubeAdjacencies[(i * 6) + 1][2] << "}, {"
        //         << cubeAdjacencies[(i * 6) + 2][0] << ", "
        //         << cubeAdjacencies[(i * 6) + 2][1] << ", "
        //         << cubeAdjacencies[(i * 6) + 2][2] << "}, {"
        //         << cubeAdjacencies[(i * 6) + 3][0] << ", "
        //         << cubeAdjacencies[(i * 6) + 3][1] << ", "
        //         << cubeAdjacencies[(i * 6) + 3][2] << "}, {"
        //         << cubeAdjacencies[(i * 6) + 4][0] << ", "
        //         << cubeAdjacencies[(i * 6) + 4][1] << ", "
        //         << cubeAdjacencies[(i * 6) + 4][2] << "}, {"
        //         << cubeAdjacencies[(i * 6) + 5][0] << ", "
        //         << cubeAdjacencies[(i * 6) + 5][1] << ", "
        //         << cubeAdjacencies[(i * 6) + 5][2] << "}";
        // }
    ...
    void draw()
    {
        ...
        rr::Mat44 m;
        rr::Vec4 light_position {{ 1.0, 3.0, 6.0, 0.0 }};
        glGetFloatv(GL_MODELVIEW_MATRIX, &m.mat[0][0]);
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glColor4ub(111, 111, 111, 111);
        drawSilhouette(m, cubeAdjacencies, light_position);
...
private:
    void searchAndAddEdge(rr::Vec3* adjacent, const rr::Vec3* verts, const uint16_t* index, const uint32_t numberOfTris, const rr::Vec3& ev0, const rr::Vec3& ev1, const uint32_t i)
    {
        for (uint32_t j = 0; j < (numberOfTris * 3); j += 3)
        {
            if (i == j) // Avoid to add the own edge to the adjacent list
                continue;
            
            const rr::Vec3 av0 = verts[index[j]];
            const rr::Vec3 av1 = verts[index[j + 1]];
            const rr::Vec3 av2 = verts[index[j + 2]];
            ...
            if (found != -1)
            {
                adjacent[0] = verts[index[j + found]];
                return;
            }
        }
    }

    void createAdjacentList(rr::Vec3* adjacent, const rr::Vec3* verts, const uint16_t* index, const uint32_t numberOfTris)
    {
        uint32_t j = 0;
        for (uint32_t i = 0; i < (numberOfTris * 3); i += 3)
        {
            const rr::Vec3 v0 = verts[index[i]];
            const rr::Vec3 v1 = verts[index[i + 1]];
            const rr::Vec3 v2 = verts[index[i + 2]];

            adjacent[(j * 6) + 0] = v0;
            adjacent[(j * 6) + 1] = v1;
            adjacent[(j * 6) + 2] = v2;

            searchAndAddEdge(&(adjacent[(j * 6) + 3]), verts, index, numberOfTris, v1, v0, i); // switch direction from v0->v1 to v1->v0 because the other triangle will have the other direction
            searchAndAddEdge(&(adjacent[(j * 6) + 4]), verts, index, numberOfTris, v2, v1, i);
            searchAndAddEdge(&(adjacent[(j * 6) + 5]), verts, index, numberOfTris, v0, v2, i);

            j++;
        }
    }

    void emitLine(const rr::Vec3& a, const rr::Vec3& b)
    {
        glBegin(GL_LINES);
        glVertex3fv(a.vec.data());
        glVertex3fv(b.vec.data());
        glEnd();
    }
    ...
    //    glDisable(GL_CULL_FACE);
    //    glBegin(GL_LINES);
    //    glVertex4f(pos1[0], pos1[1], pos1[2], 1.0f);
    //    glVertex4f(pos2[0], pos2[1], pos2[2], 0.0f);
    //    glVertex4f(pos3[0], pos3[1], pos3[2], 1.0f);
    //    glVertex4f(pos4[0], pos4[1], pos4[2], 0.0f);
    //    glEnd();
    ...
    void drawSilhouette(rr::Mat44 m, const rr::Vec3* verts, const rr::Vec4& lightPos)
    {
        rr::Mat44 mInv = m;
        mInv.invert();
        rr::Vec4 tmpLightPos;
